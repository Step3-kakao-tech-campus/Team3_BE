...
> ``기존의 볼링 관련 서비스``에서는 ``볼링 한판``을 치기 위해서 동호회, 소모임에 가입을 해야하는  ``번거로운 과정``이 필요합니다.
>
> 소모임, 밴드 앱 또한 일회성의 가벼운 만남이 아닌 주기적으로 참여를 할 회원을 모집하고 있습니다.
>
...
2. Temp3_BE/.env.example 파일을 .env로 변경하고 내용을 자신의 환경에 맞게 설정해준다.
...
3. .env 환경변수 등록 (본 가이드에선 우분투 환경으로 진행)
...
4. docker-compose.yml 파일을 이용해 Redis 및 MySQL 도커 컨테이너를 실행한다.(도커가 설치되어 있다고 가정)
...
>
> **회원가입을 하지 않은 유저는 아래의 기능만 사용이 가능하다.(주로 조회만 가능)**
> - 행정구역 관련 기능
> - 모집글 조회
> - 댓글 조회
...
> - 이후 모집글을 베이스로 참여기록, 별점, 점수등록 등이 이루어 지기 때문에모임 확정 이후에는 모집글 내용을 수정하거나 삭제할 수 없다.
...
> - 커서 기반 페이징을 하였다.
> - 삭제된 댓글에 대댓글이 있는 경우만, "삭제된 댓글입니다"/ 유저id null로 돌려준다.
    그외, 대댓글이 없는 삭제된 댓글과 대댓글이 삭제된 경우의 댓글은 responseBody에 포함되어 내려가지 않는다.
> - 삭제된 댓글들로 인한 통일되지 않은 응답 개수 문제는, 프론트의 구현이 무한 스크롤로 이루어질 것이다.
    따라서 적은 개수가 응답되어도 자동으로 다음 key로 요청이 일어나 사용자 경험에 큰 영향을 주지 않는다.
    그렇기 때문에 무시해도 될 것 같아 이렇게 구현하였다.
...
> - 업로드 가능한 확장자는 png, jpg, jpeg, gif 4가지이다.
